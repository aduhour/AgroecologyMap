...
![Pósters presentados al congreso](img/posterscpa.JPG)
...
almacen <- subset(almacen, subset = almacen$shop == "convenience")

prod <- agroecology$osm_points
prod <- subset(prod, subset = prod$place == "farm")

feria = agroecology$osm_points
feria = subset(feria, subset = feria$amenity == "marketplace")

map <- tm_shape(area) + tm_borders() +
tm_shape(almacen) + tm_symbols(fill = "black",shape = 1)+
  tm_shape(feria) + tm_symbols(fill = "blue", shape = 2)+
  tm_shape(prod) + tm_symbols(fill = "green")+
  tm_basemap() + tm_scalebar()
map
```
## Mapa de Luján
